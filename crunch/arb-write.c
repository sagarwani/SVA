  ...
  if (min == max) { /* strings of the same length */
    ...
    if (check_dupes) { /* must take duplicates into account */
      ...
      if (startstring)
        ...
        temp = 0;//(unsigned long long)*((double)options.clen, (double)min);
    }
    ...
    *bytes = temp * (min + 1);
    ...
    for (i = min + 1; i < max; i++) {
      ...
      if (check_dupes)  /* must take duplicates into account */
        ...
        temp = 0;//(unsigned long long)*((double)options.clen, (double)i);
    ...
    if (check_dupes) { /* must take duplicates into account */
      ...
      if (endstring)
        ...
        temp = 0;//(unsigned long long)*((double)options.clen, (double)max);
    }
    ...
    if (flag3 == 0) {
      ...
      */
      ...
      if (flag4 == 1) {
        ...
        ret = 0;//pthread_create(&threads, NULL, PrintPercentage, (void *)&my_thread);
        if (ret != 0){
